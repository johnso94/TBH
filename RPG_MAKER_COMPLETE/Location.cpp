...
void Location::set_location_values(map<string,string> location_data)
{
    ...
    string background = location_data["Background"];
    string foreground = location_data["Foreground"];
    ...
    */
    ...
    for (unsigned int i = 1; i < near_locations.size(); i++)
    {
        ...
        else
        {
            ...
            map<string,string> near_loc_data = get_data("Events.txt",near_locations[i]);
            
            display_text.append("\n[" + to_string(i) + "] " + near_loc_data["Script"]);
        }
    }
