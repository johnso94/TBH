...
void Client::run()
{
    string current_event = get_value("Game.txt","Event:");
...
void Client::navigation(map<string,string> location_data)
{
    // Reading in from Txt files to create location
    Location location;
    location.name = location_data["Name"];
    location.has_back_location = location_data["Back"];
    location.description = location_data["Description"];
    location.near_locations = get_near_locations(location.name);
    ...
    if(user_input != -1)
    {
        ...
        set_value(game_file,"Background:",new_current_event+"_B.png");
        set_value(game_file,"Forground:",new_current_event+"_F.png");
    } 
    else 
    {
        cout << "\nInvalid Input\n\n";
...
void Client::combat(map<string,string> monster_data)
{

    printf("### %s ###\n",monster_data["Name"].c_str());
    printf("Str: %i\n",stoi(monster_data["Str"]));
    printf("Def: %i\n",stoi(monster_data["Def"]));
    printf("Hp: %i\n",stoi(monster_data["Hp"]));
}

vector<string> Client::split(string str, char c)
{
...
string Client::RNG_calculator(string _object, string _chance)
{
    vector<string>object = split(_object, '/');
    vector<string>chance = split(_chance, '/');

    cut_spaces_off_ends(object);
    cut_spaces_off_ends(chance);

    vector<pair<string,int> > object_and_chance;

    int max_roll = 0;
    for(unsigned int i = 0; i < object.size(); i++)
    {
        max_roll += stoi(chance[i]);
        object_and_chance.push_back({object[i],max_roll});
    }
    
    srand(time(NULL));
    int roll = rand() % max_roll;

    string chosen_object = "__NONE__";
    for(unsigned int i = 0; i < object_and_chance.size(); i++)
    {
        if(roll <= object_and_chance[i].second)
        {
            chosen_object = object_and_chance[i].first;
            break;
        }
    }
...
{
    ifstream file_in(locations_file);
...
vector<string> Client::get_near_locations(string current_location)
{
...
void Client::cut_spaces_off_ends(vector<string> &v)
{
    for(auto &elem : v)
    {
